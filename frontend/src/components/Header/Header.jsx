...
const Header = (props) => {
	...
	console.log(artisansByName)
	return (
		<header className="shadow mb-4 bg-white">
			<div className="container-lg">
				<nav className="navbar navbar-expand-lg nav-underline p-0">
					{/* navbar */}
					...
					<div className="container-fluid">
						<div className="container-fluid p-0">
							...
							>
								<div>
									<ul className="nav navbar-nav nav-underline  mb-2 mb-lg-0">
										{props.categories.map((category) => {
											return (
											<li className="nav-item m-2" key={category.id}>
												<NavLink to={`/categories/${category.id}`} className="nav-link text-black " >{category.name}</NavLink>
											</li>)
										})}
									</ul>
								</div>
